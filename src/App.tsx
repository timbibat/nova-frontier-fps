...
export default function App() {
  ...
  if (!isJoined) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-900 text-white font-sans">
        <div className="p-8 bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 w-full max-w-md">
          <h1 className="text-4xl font-bold mb-6 text-center tracking-tighter bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent uppercase">
            Nova Frontier
          </h1>
          <form onSubmit={handleJoin} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">PILOT CALLSIGN</label>
              <input
                id="player-name"
                type="text"
                value={playerName}
                ...
                className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none transition-all text-white"
                autoFocus
                autoComplete="off"
              />
            </div>
            <button
              id="join-btn"
              type="submit"
              className="w-full py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-lg transition-colors shadow-lg shadow-cyan-900/20 uppercase tracking-widest"
            >
              Initialize Combat
            </button>
          </form>
          <div className="mt-6 flex justify-center gap-4 grayscale opacity-50">
             <div className="text-[10px] uppercase border border-slate-700 px-2">Sector_01</div>
             <div className="text-[10px] uppercase border border-slate-700 px-2">Bot_Systems_Online</div>
          </div>
        </div>
      </div>
    );
  ...
  if (!gameState || !myId) return <div className="h-screen flex items-center justify-center bg-slate-900 text-cyan-500 animate-pulse font-mono">ESTABLISHING NEURAL LINK...</div>;
    ...
    <div className="h-screen w-screen bg-black overflow-hidden relative">
      ...
      {isPaused && (
        <div className="absolute inset-0 flex items-center justify-center bg-slate-950/80 backdrop-blur-md z-50">
          <div className="p-8 bg-slate-900 border border-amber-500/40 rounded-2xl shadow-2xl shadow-amber-500/5 max-w-md w-full font-mono text-white animate-fade-in relative overflow-hidden">
